...
3. 必要なら `Specify rest pose frame` をオンにし、`Rest pose frame` にデフォルトポーズとするフレームを入れます（範囲外可。optionVar `ARFHUnityFbxRestFrame` / `ARFHUnityFbxRestFrameEnabled` に保存）。
...
- デフォルトポーズ（`rest_frame`。未指定なら開始フレーム）: FBX の Model の `Lcl Translation/Rotation/Scaling` には、書き出し時点の現在フレームの値が入ります。Unity の Humanoid Avatar はこの値を基準にします。範囲外のキーは後で削除するため、加工前のリグで `rest_frame` の TRS・ローカル行列・表情ウェイトを記録しておきます。`rest_frame` が範囲外の場合は、キー付きチャンネルの最初のキー（範囲より後ろなら最後のキー）を fixed タンジェントにし、範囲外側のタンジェント角と linear infinity を調整して、そのフレームで記録値になるようにします。範囲外側のタンジェントはクリップとして書き出されないため、キーは増えません。範囲内側のタンジェント角は元の値に固定し、隣のキーまでの曲線が変わらないことを確認します。そのうえで `currentTime(rest_frame)` にして書き出します。ベイクで Euler 値が ±360° 変わることがあるため、ポーズの再現はローカル行列で照合します。
- 書き出した FBX バイナリを直接読み（`_read_fbx_defaults`）、関節の `Lcl` 値が `rest_frame` の値と一致するか確認します。Maya で再読み込みすると値がアニメーションで上書きされるため、再読み込みでは確認できません。FBX は既定値（移動・回転 0、スケール 1）と等しいプロパティを省略します。
